...
def answer_two():
    original =  len(df1)
    df4= pd.merge(ScimEn, energy, how='inner', left_on = 'Country', right_on = 'Country')
    df5 = pd.merge(df4,GDP,how='inner', left_on = 'Country', right_on = 'Country')  
    return original - len(df5)
answer_two()
...
def answer_four():  
    Top15 = answer_one()
    three = answer_three()
    numsix = three.index[5]
    change = Top15.loc[numsix]['2015'] - Top15.loc[numsix]['2006']
    return change
answer_four()
...
def answer_five():
    Top15 = answer_one()    
    avgsup = Top15['Energy Supply per Capita'].mean()
    return avgsup
answer_five()                       
...
def answer_six():
    Top15 = answer_one()
    maxre = Top15['% Renewable'].idxmax(), Top15['% Renewable'].max()
    return maxre
answer_six()
...
def answer_seven():
    Top15 = answer_one()
    Top15['ratiocitat'] = Top15['Self-citations'] / Top15['Citations']
    answerseven = Top15['ratiocitat'].idxmax(),Top15['ratiocitat'].max()
    return answerseven
answer_seven()
...
def answer_eight():
    ...
    eight = Top15['popest'].sort_values(ascending=False).index[2]
    return eight
answer_eight()
...
def answer_nine():
    Top15 = answer_one()
    Top15['popest'] = Top15['Energy Supply'] / Top15['Energy Supply per Capita']
    Top15['citable_person'] = (Top15['Citable documents'] / Top15['popest']).astype(float)
    return Top15['citable_person'].corr(Top15['Energy Supply per Capita'])
answer_nine()
...
def answer_eleven():
    ...
    eleven = Top15.set_index('Continent').groupby(level = 0)['Population Estimate'].agg({'size':np.size, 'sum':np.sum, 'mean':np.mean, 'std':np.std})
    return eleven
